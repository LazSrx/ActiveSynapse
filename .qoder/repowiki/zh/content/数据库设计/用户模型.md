...
## 更新摘要
**变更内容**
- 修复了用户模型中的 Float 导入问题，确保数据库列定义的完整性
- 新增头像上传功能支持
- 更新用户更新模式包含头像字段
- 新增文件上传配置参数
- 完善用户信息管理的完整工作流
...
- UserProfile 表（user_profiles）
  ...
  - 字段
    - height_cm：整型，厘米，可空
    - weight_kg：**浮点型**，千克，可空（**已修复 Float 导入问题**）
    - birth_date：日期时间，可空
...
- **Float 字段精度**
  - **体重字段已正确使用浮点型，确保小数精度的准确性，避免数据截断问题**
...
- **体重数据异常**
  - **如遇体重数据被截断为整数的问题，确认 Float 字段已正确导入并使用**
...
## 结论
用户模型围绕 User 与 UserProfile 两大实体展开，采用一对一强关联与多对多弱关联相结合的方式，既保证了用户档案的完整性，又为运动、营养、损伤等子域提供了扩展空间。通过异步数据库、Pydantic 模式与 JWT 认证，系统在安全性、可维护性与性能之间取得平衡。新增的头像上传功能进一步完善了用户信息管理，配合灵活的文件上传配置，为未来的实际存储实现奠定了基础。

**重要更新**：本次更新修复了用户模型中的 Float 导入问题，确保 UserProfile 中的体重字段（weight_kg）能够正确使用浮点型数据类型，避免了小数精度丢失的问题。这一修复提升了数据存储的准确性，特别是对于需要精确体重记录的用户群体。

建议后续根据查询热点对 JSON 字段进行规范化或物化，进一步优化检索效率。
